...
// 转换到-pi-pi
#define constrain_npi_pi(amt) (((amt) > PI) ? ((amt) -= 2 * PI) : (((amt) < -PI) ? ((amt) += 2 * PI) : (amt)))

// 转换到0-360
#define constrain_0_360(amt) (((amt) > 360) ? ((amt) -= 360) : (((amt) < 0) ? ((amt) += 360) : (amt)))
// 转换到-180-180
#define constrain_n180_180(amt) (((amt) > 180) ? ((amt) -= 360) : (((amt) < -180) ? ((amt) += 360) : (amt)))

/* 限幅宏 */
// 下限限幅
#define constrain_mini(amt, low) ((amt) < (low) ? ((amt) = (low)) : (amt))
// 上限限幅
#define constrain_max(amt, high) ((amt) > (high) ? ((amt) = (high)) : (amt))
// 限幅
#define constrain(amt, low, high) ((amt) < (low) ? ((amt) = (low)) : ((amt) > (high) ? ((amt) = (high)) : (amt)))
// 循环限幅
#define loop_constrain(amt, low, high) ((amt) < (low) ? ((amt) = (high)) : ((amt) > (high) ? ((amt) = (low)) : (amt)))
...
#define HIGH    0x1
#define LOW     0x0
#define FLOAT_ZERO  1e-6  // 当变量的绝对值小于此时，变量float为0
